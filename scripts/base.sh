...
sudo DEBIAN_FRONTEND=noninteractive apt-get install -y -qq curl unzip git-core ack-grep software-properties-common build-essential cachefilesd
...
sudo openssl req -new -subj "/C=GB/ST=London/L=London/O=Global Security/OU=IT Department/CN=$DOMAIN" -key "$SSL_DIR/xip.io.key" -out "$SSL_DIR/xip.io.csr" -passin pass:$PASSPHRASE
